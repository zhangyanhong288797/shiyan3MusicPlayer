...
public class MusicService extends Service implements MediaPlayer.OnErrorListener, MediaPlayer.OnPreparedListener, MediaPlayer.OnCompletionListener, OnAudioFocusChangeListener {
            ...
            public void run() {
                try {
                    if (mPlayer.isPlaying()) {
                        ...
                        int totalDuration = mPlayer.getDuration();//总时长
                        mMessage = Message.obtain();
    ...
    public void onCompletion(MediaPlayer mediaPlayer) {
        Intent intent = new Intent();
        intent.setAction(Constants.ACTION_NEXT);//隐式启动
        sendBroadcast(intent);//调用此方法将intent发送后，该intent会被发送到system process进程。该进程会对收到的intent检查与注册的所有广播中的action是否相同，若相同则会建立映射表。并将该intent转发给所对应的广播接收器。
    }
